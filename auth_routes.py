...
async def get_google_auth_url():
    """Get Google OAuth authorization URL for login with Google account."""
    ...
    authorization_url, state = google_service.build_google_auth_url()
    return {
        "status_code": 200,
        "success": True,
...
async def google_callback(request: GoogleCallbackRequest):
    ...
    Returns JWT plus the user's Google Chrome bookmarks and browsing history.
    """
    if not google_service.is_google_oauth_configured():
    ...
    try:
        ...
        if not google_sub or not email:
            raise HTTPException(status_code=400, detail="Google account missing required profile information")
    ...
    if tokens.get("refresh_token"):
        try:
            ...
            await auth_service.store_oauth_tokens(
                ...
                scopes=tokens.get("scopes") or settings.google_oauth_scopes,
            )
        except Exception as exc:
